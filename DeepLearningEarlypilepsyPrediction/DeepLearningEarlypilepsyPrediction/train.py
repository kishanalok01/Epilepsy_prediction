...
sys.path.append('/home/ksaab/Documents/eeg_fully_supervised/model/')

import model.data_loader as data_loader
import model.data_loader_both as data_loader_both


from evaluate import evaluate
from evaluate_model import get_scores

from torch.optim.lr_scheduler import MultiStepLR
parser = argparse.ArgumentParser()

parser.add_argument('--model_dir', default='experiments_KS_lpch/dump', help="Directory containing params.json")
parser.add_argument('--files_dir', default='file_markers_lpch_small/', help="Directory containing txt file of listed names")
parser.add_argument('--files_dir2', default='file_markers_stanford_small/', help="Directory containing txt file of listed names")
...
parser.add_argument('--lr', default=0, help="exponent for learning rate",type=int)
...
parser.add_argument('--data_dir2', default= 'lfs/1/jdunnmon/eeg/EEG/eegdbs/SEC/stanford', help="Directory containing the dataset")

parser.add_argument("-v", "--verbose", action="store_true", help="increase output verbosity")

parser.add_argument("-g", "--gold", action="store_true", help="run gold test")
...
#                 break
            ...
            if train_batch.shape != torch.Size([params.batch_size, 200*params.clip_len, 19]):
                #print("SKIPPING")
                continue
...
def train_and_evaluate(model, train_dataloader, val_dataloader, test_dataloader, optimizer, loss_fn, metrics, params, model_dir,scale_ratio,restore_file=None):
    ...
    max_epochs = np.min([75,int(params.num_epochs/scale_ratio)])
    ...
    for epoch in range(max_epochs):
        ...
        if not(terminate):
            ...
            val_metrics,roc_curve_arrays = get_scores(model, loss_fn, val_dataloader)
    ...
    test_metrics,test_roc_curve_arrays = get_scores(model, loss_fn, test_dataloader)
...
#     os.environ["CUDA_VISIBLE_DEVICES"]="1"
    ...
    print('Clip length: ',int(params.clip_len))
...
#     if params.cuda: torch.cuda.manual_seed(230)
    ...
    if args.dataloader == 0:
        dataloaders = data_loader.fetch_dataloader(['train', 'val','test'], args.data_dir, args.files_dir, batch_num = params.batch_size, clip_length= params.clip_len, num_workers = params.num_workers, scale_ratio=args.scale_ratio,gold=args.gold)
    if args.dataloader == 1:
        # for loading both lpch and stanford
        dataloaders = data_loader_both.fetch_dataloader(['train', 'val','test'], args.data_dir, args.data_dir2,  args.files_dir, args.files_dir2, batch_num = params.batch_size, clip_length= params.clip_len, num_workers = params.num_workers)
